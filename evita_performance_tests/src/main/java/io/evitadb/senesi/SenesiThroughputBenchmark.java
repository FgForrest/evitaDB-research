...
public class SenesiThroughputBenchmark extends SenesiBenchmark {
	...
	@Threads(Threads.MAX)
	public void memTableRead(InMemorySingleReadSenesiState state, Blackhole blackhole) {
		final EvitaSession session = (EvitaSession) state.getSession();
		final PrimaryKey primaryKey = (PrimaryKey) FinderVisitor.findConstraint(
			state.getQuery().getFilterBy(), filterConstraint -> filterConstraint instanceof PrimaryKey
		);
		blackhole.consume(
			session.getEntity("Product", primaryKey.getPrimaryKeys()[0], entityBody(), attributes())
		);
	}
