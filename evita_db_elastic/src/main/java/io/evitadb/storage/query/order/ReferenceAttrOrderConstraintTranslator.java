 ...
 **/
public abstract class ReferenceAttrOrderConstraintTranslator<C extends OrderConstraint> extends AbstractOrderConstraintTranslator<C> {
    ...
    protected EsOrderConstraint applyInternally(String attributeName, OrderByVisitor orderByVisitor) {
        ...
        return new EsOrderConstraint(
                new FieldSortBuilder(
                        "references.sortRefs.valueToSort.keyword")
                        .setNestedSort(
                                new NestedSortBuilder("references")
                                        .setFilter(
                                                matchQuery(
                                                        "references.simpleType.keyword",
                                                        new EsEnumWrapper<>(orderByVisitor.getReferenceSchema().getEntityType()).getValue()
                                                ).operator(Operator.AND)
                                        )
                                        .setNestedSort(
                                                new NestedSortBuilder("references.sortRefs")
                                                        .setFilter(
                                                                matchQuery(
                                                                        "references.sortRefs.combinedField.keyword",
                                                                        attributeName
                                                                ).operator(Operator.AND)
                                                        )
                                        ))
