...
import io.evitadb.api.query.filter.LessThanEquals;
 ...
 **/
public class LessThanEqualsTranslator extends AttributeFilterConstraintTranslator<LessThanEquals> {

    @SneakyThrows
    @Override
    public EsConstraint applyInternally(LessThanEquals constraint, FilterByVisitor filterByVisitor) {

        final AttributeSchema attributeSchema = getAttributeSchema(filterByVisitor, constraint.getAttributeName());
        Assert.isTrue(!attributeSchema.getType().isArray(), "Constraint lessThan does not support array values filtering.");
        Object serializedValue = convertObjectAndSerializeValue(constraint.getAttributeValue(),filterByVisitor);
        ...
        BoolQueryBuilder query = buildDefaultQuery(rowName, constraint.getAttributeName(),
                boolQuery().must(
                        rangeQuery(filterByVisitor.getPathPrefix() + "attribute.value."+ rowName).lte(serializedValue).gte(null)
                ), filterByVisitor);
