 ...
 **/
public class LessThanTranslator extends AttributeFilterConstraintTranslator<LessThan> {

    @SneakyThrows
    @Override
    public EsConstraint applyInternally(LessThan constraint, FilterByVisitor filterByVisitor) {
        ...
        BoolQueryBuilder query = buildDefaultQuery(rowName, constraint.getAttributeName(),
                boolQuery().must(
                        rangeQuery(filterByVisitor.getPathPrefix() + "attribute.value."+ rowName).lt(serializedValue).gt(null)
                ), filterByVisitor);
