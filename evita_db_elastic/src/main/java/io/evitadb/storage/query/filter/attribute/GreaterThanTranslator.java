...
import io.evitadb.api.query.filter.GreaterThan;
 ...
 **/
public class GreaterThanTranslator extends AttributeFilterConstraintTranslator<GreaterThan> {

    @SneakyThrows
    @Override
    public EsConstraint applyInternally(GreaterThan constraint, FilterByVisitor filterByVisitor) {
        ...
        BoolQueryBuilder query = buildDefaultQuery(rowName, constraint.getAttributeName(),
                boolQuery().must(
                        rangeQuery(filterByVisitor.getPathPrefix() + "attribute.value."+ rowName).gt(serializedValue).lt(null)
                ), filterByVisitor);
