...
public class GreaterThanEqualsTranslator extends AttributeFilterConstraintTranslator<GreaterThanEquals> {
    ...
    @SneakyThrows
    @Override
    public EsConstraint applyInternally(GreaterThanEquals constraint, FilterByVisitor filterByVisitor) {
        ...
        BoolQueryBuilder query = buildDefaultQuery(
                ...
                boolQuery().must(
                        rangeQuery(filterByVisitor.getPathPrefix() + "attribute.value." + rowName).gte(serializedValue).lte(null)
                ), filterByVisitor);
