...
/**
 * This {@link Serializer} implementation reads/writes {@link InSet} from/to binary format.
 *
 * @author Jan Novotný (novotny@fg.cz), FG Forrest a.s. (c) 2022
 */
@RequiredArgsConstructor
public class InSetSerializer<T extends Serializable & Comparable<?>> extends Serializer<InSet> {
	private final KryoSerializationHelper kryoSerializationHelper;

	@Override
	public void write(Kryo kryo, Output output, InSet object) {
		...
		for (Object comparableValue : set) {
			kryoSerializationHelper.writeSerializable(kryo, output, (Serializable) comparableValue);
		}
	}

	@SuppressWarnings("unchecked")
	@Override
	public InSet read(Kryo kryo, Input input, Class<? extends InSet> type) {
		...
		for (int i = 0; i < setSize; i++) {
			set[i] = (T) kryoSerializationHelper.readSerializable(kryo, input);
		}
