...
public class HeterogeneousMapSerializer<K, V> extends Serializer<Map<K, V>> {
	...
	public void write(Kryo kryo, Output output, Map<K, V> object) {
		...
		for (Entry<K, V> entry : object.entrySet()) {
			kryoSerializationHelper.writeSerializable(kryo, output, (Serializable)entry.getKey());
			kryoSerializationHelper.writeSerializable(kryo, output, (Serializable)entry.getValue());
		}
	}
	...
	public Map<K, V> read(Kryo kryo, Input input, Class<? extends Map<K, V>> type) {
		...
		for (int i = 0; i < itemCount; i++) {
			...
			targetSet.put(
				(K)kryoSerializationHelper.readSerializable(kryo, input),
				(V)kryoSerializationHelper.readSerializable(kryo, input)
			);
		}
		return targetSet;
	}
