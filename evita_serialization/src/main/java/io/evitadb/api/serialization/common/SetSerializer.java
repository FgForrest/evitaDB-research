...
public class SetSerializer<T extends Serializable> extends Serializer<Set<T>> {
	...
	public void write(Kryo kryo, Output output, Set<T> object) {
		...
		for (T itemInSet : object) {
			if (!classWritten) {
				kryoSerializationHelper.writeSerializableClass(kryo, output, itemInSet.getClass());
				classWritten = true;
			}
			kryoSerializationHelper.writeObject(kryo, output, itemInSet);
		}
	}
	...
	public Set<T> read(Kryo kryo, Input input, Class<? extends Set<T>> type) {
		...
		for (int i = 0; i < itemCount; i++) {
			if (itemClass == null) {
				itemClass = kryoSerializationHelper.readSerializableClass(kryo, input);
			}
			targetSet.add(kryoSerializationHelper.readObject(kryo, input, itemClass));
		}
		return targetSet;
	}
