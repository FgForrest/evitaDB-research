...
public class SerialVersionBasedSerializer<T> extends Serializer<T> {
	...
	public SerialVersionBasedSerializer(Serializer<T> currentVersionSerializer, Class<T> targetClass) {
		try {
			final Field serialVersionUID = targetClass.getDeclaredField("serialVersionUID");
			serialVersionUID.setAccessible(true);
			this.currentSerializerUID = (long) serialVersionUID.get(null);
			this.currentVersionSerializer = currentVersionSerializer;
		} catch (NoSuchFieldException | IllegalAccessException e) {
			throw new IllegalStateException(targetClass.getSimpleName() + " class has no static serialVersionUID field!");
		}
	}
	 ...
	 * @param backwardCompatibleSerializer
	 */
	public void addBackwardCompatibleSerializer(long serialVersionUID, Serializer<T> backwardCompatibleSerializer) {
		final Map<Long, Serializer<T>> serializerIndex = ofNullable(backwardCompatibleSerializers)
				.orElseGet(() -> {
					this.backwardCompatibleSerializers = new HashMap<>();
	...
	public T read(Kryo kryo, Input input, Class<? extends T> type) throws StoredVersionNotSupportedException {
		...
		// fast line - this will be most common way
