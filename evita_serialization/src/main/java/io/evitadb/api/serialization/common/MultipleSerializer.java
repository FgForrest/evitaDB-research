...
public class MultipleSerializer extends Serializer<Multiple> {
	...
	public Multiple read(Kryo kryo, Input input, Class<? extends Multiple> type) {
		...
		for (int i = 0; i < length; i++) {
			values[i] = kryoSerializationHelper.readSerializable(kryo, input);
		}
		if (values.length == 2) {
			return new Multiple(
			...
			throw new IllegalArgumentException("Currently only 2 to 4 arguments for multiple are supported!");
		}
	}
