...
public class ArrayUtils {
	...
	public static int[] insertIntIntoOrderedArray(int recId, int[] recordIds) {
		...
		return position.isAlreadyPresent() ? recordIds : insertIntIntoArrayOnIndex(recId, recordIds, position.getPosition());
	}
	...
	public static long[] insertLongIntoOrderedArray(long recId, long[] recordIds) {
		...
		return position.isAlreadyPresent() ? recordIds : insertLongIntoArrayOnIndex(recId, recordIds, position.getPosition());
	}
	...
	public static <T extends Comparable<T>> T[] insertRecordIntoOrderedArray(T newItem, UnaryOperator<T> mutator, T[] records) {
		...
		final int posIndex = position.getPosition();
	...
	public static class InsertionPosition {
		/**
		 * Position where the looked up object:
		 * - currently is (when alreadyPresent=true)
		 * - should potentially be (when alreadyPresent=false)
		 */
		private final int position;
		/**
		 * Holds true if looked up object was found already present in the array.
		 */
