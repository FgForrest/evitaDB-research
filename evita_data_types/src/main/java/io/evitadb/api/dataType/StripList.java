...
@EqualsAndHashCode
@ToString
public class StripList<T extends Serializable> implements DataChunk<T> {
	...
	@Override
	public <U extends Serializable> StripList<U> replaceData(List<U> newContent) {
		Assert.isTrue(data.size() == newContent.size(), "Old and new content should be of equal size!");
		return new StripList<>(
			offset, limit, totalRecordCount, newContent
		);
	}
