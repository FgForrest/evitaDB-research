...
public class Trie<T extends Serializable> implements Serializable {
	private static final long serialVersionUID = 7649813495448701492L;
	final Class<T> type;
	...
	Trie(Class type, TrieNode<T> rootNode) {
		this.type = type;
		//noinspection unchecked
		this.arrayFactory = length -> (T[]) Array.newInstance(type, length);
		this.root = rootNode;
	 ...
	 */
	public Set<String> getWordsBetween(@Nonnull final String prefix, char... breakingCharacter) {
	 ...
	 * @param breakingCharacter breaking character that would finish our search deepwise (in example `]`)
	 */
	public Set<String> getWordsBetweenOrStartingWith(@Nonnull final String prefix, char... breakingCharacter) {
	 ...
	 */
	private Set<String> getWordsBetween(@Nonnull final String prefix, char[] breakingCharacter, boolean optionalBreakingCharacter) {
		final StringBuilder pathBuilder = new StringBuilder();
		final TrieNode<T> prefixRootNode = findPrefixRoot(prefix, pathBuilder);
		if (prefixRootNode == null) {
			return Collections.emptySet();
	 ...
	 */
	public Set<Entry<String, T[]>> wordToValuesEntrySet() {
		final Set<Entry<String, T[]>> result = new HashSet<>();
		walkTree(
			root,
			new StringBuilder(),
