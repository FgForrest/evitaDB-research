...
/**
 * This test verifies contract of {@link TransactionalMemoryList} implementation.
 *
 * @author Jan Novotný (novotny@fg.cz), FG Forrest a.s. (c) 2018
 */
class TransactionalMemoryListTest {
	private List<Integer> underlyingData;
	private TransactionalMemoryList<Integer> tested;
	...
	void setUp() {
		...
		tested = new TransactionalMemoryList<>(underlyingData);
	}
	...
	void verify() {
		...
		tested = new TransactionalMemoryList<>(base);
		assertStateAfterCommit(
			tested,
			original -> {
				original.add(4, 3);
	...
	void generationalProofTest() {
		...
		final List<Integer> initialList = generateRandomInitialArray(rnd, initialCount);
		final AtomicReference<TransactionalMemoryList<Integer>> transactionalList = new AtomicReference<>(new TransactionalMemoryList<>(initialList));
		final List<Integer> referenceList = new ArrayList<>(initialList);
		...
		try {
			...
			do {
				assertStateAfterCommit(
					transactionalList.get(),
					original -> {
						sb.setLength(0);
						final TransactionalMemoryList<Integer> txList = transactionalList.get();
						sb.append("\nSTART: " + txList.stream().map(Object::toString).collect(Collectors.joining(",")) + "\n");

						final int operationsInTransaction = rnd.nextInt(5);
						for (int i = 0; i < operationsInTransaction; i++) {
							final int length = txList.size();
							...
							if ((operation == 0 || length < 10) && length < 120) {
								if (rnd.nextBoolean()) {
									...
									final int addIndex = rnd.nextInt(length - 1);
									final Integer newRecId = rnd.nextInt(initialCount * 2);
									txList.add(addIndex, newRecId);
									referenceList.add(addIndex, newRecId);
									sb.append("++(" + addIndex + ")" + newRecId);
								}
							} else if (operation == 1) {
								if (rnd.nextBoolean()) {
									// remove existing item by index
									final int removeIndex = rnd.nextInt(length);
									sb.append("-" + removeIndex);
									txList.remove(removeIndex);
									referenceList.remove(removeIndex);
								} else {
									// remove existing item by value
									final Integer removedRecId = txList.get(rnd.nextInt(length));
									txList.remove(removedRecId);
									referenceList.remove(removedRecId);
									sb.append("--" + removedRecId);
								}
							} else {
								// remove existing item by index
								...
								txList.set(updateIndex, updatedValue);
					...
					(original, committed) -> {
						...
						transactionalList.set(new TransactionalMemoryList<>(new ArrayList<>(referenceList)));
					}
				);
				if (iteration++ % 1000 == 0) {
					System.out.print(".");
