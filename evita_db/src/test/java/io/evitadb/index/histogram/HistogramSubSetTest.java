...
class HistogramSubSetTest {
	...
	private final HistogramSubSet<Integer> tested = new HistogramSubSet<>(
		1L,
		new HistogramBucket[]{
			new HistogramBucket(1, 1, 2),
			new HistogramBucket(5, 8, 9),
		},
		(indexTransactionId, histogramBuckets) -> new OrFormula(
			new long[]{indexTransactionId}, Arrays.stream(histogramBuckets).map(HistogramBucket::getRecordIds).toArray(RoaringBitmapBackedBitmap[]::new)
		)
	);
