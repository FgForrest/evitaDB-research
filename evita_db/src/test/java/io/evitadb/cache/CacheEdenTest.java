	...
	private static PriceRecordContract[] generateRandomPriceRecords(int number) {
		...
		for (int i = 0; i < number; i++) {
			...
			if (random.nextBoolean()) {
				...
				final boolean withVat = random.nextBoolean();
				result[i] = new CumulatedVirtualPriceRecord(
					entityPrimaryKey,
					price,
					withVat ? QueryPriceMode.WITH_VAT : QueryPriceMode.WITHOUT_VAT
				);
			}
		}
		Arrays.sort(result, Comparator.comparingLong(PriceRecordContract::getEntityPrimaryKey));
		return result;
	}
