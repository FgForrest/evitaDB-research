...
class HistogramDataCruncherTest {
	...
	void computeHistogramFromDistinctValuesWithLessStepsAndSpecificWeight() {
		final HistogramDataCruncher<HistogramBucket<Integer>> cruncher = createHistogramBucketCruncher(
			3,
			new HistogramBucket<>(100, 1, 11, 12, 13, 14, 15, 16, 17, 18, 19),
			new HistogramBucket<>(200, 2, 21),
			new HistogramBucket<>(300, 3, 31, 32, 33, 34),
			new HistogramBucket<>(400, 4, 41, 42),
			new HistogramBucket<>(500, 5, 51, 52, 53, 54, 55)
		);
		assertArrayEquals(
			new Bucket[]{
	...
	@Nonnull
	private static HistogramDataCruncher<HistogramBucket<Integer>> createHistogramBucketCruncher(int stepCount, HistogramBucket<Integer>... data) {
		return new HistogramDataCruncher<>(
			stepCount, 0,
			data,
			HistogramBucket::getValue,
