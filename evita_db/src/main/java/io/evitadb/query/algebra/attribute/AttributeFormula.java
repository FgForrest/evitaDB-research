...
package io.evitadb.query.algebra.attribute;
 ...
 * computation that needs to exclude constraint targeting the attribute histogram is computed for.
 *
...
public class AttributeFormula extends AbstractCacheableFormula {
	...
	/**
	 * Contains {@link AttributeSchema#getName()} of the attribute that is targeted by inner constraint.
	 */
	...
	public Formula getCloneWithInnerFormulas(@Nonnull Formula... innerFormulas) {
		Assert.isTrue(innerFormulas.length == 1, () -> new IllegalArgumentException(ERROR_SINGLE_FORMULA_EXPECTED));
	...
	protected Bitmap computeInternal() {
		if (innerFormulas.length == 0) {
			...
			throw new IllegalStateException(ERROR_SINGLE_FORMULA_EXPECTED);
		}
	}
