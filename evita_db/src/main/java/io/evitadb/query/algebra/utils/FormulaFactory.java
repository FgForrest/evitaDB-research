...
package io.evitadb.query.algebra.utils;
...
public class FormulaFactory {
	...
	/**
	 * Creates boolean OR constraint for passed `innerFormulas`. If the array is empty, it returns `emptyFormula`.
	...
	/**
	 * Creates boolean AND constraint for passed `innerFormulas`. If the array is empty, it returns `emptyFormula`.
	...
	/**
	 * Creates boolean NOT constraint for passed formulas, either for integer or long type.
	 */
	...
	private static Formula[] getMergedAndFormulas(Formula... formulas) {
		final CompositeObjectArray<Formula> mergedFormulas = new CompositeObjectArray<>(Formula.class);
		for (Formula innerFormula : formulas) {
			if (innerFormula instanceof io.evitadb.query.algebra.base.AndFormula) {
				mergedFormulas.addAll(innerFormula.getInnerFormulas(), 0, innerFormula.getInnerFormulas().length);
