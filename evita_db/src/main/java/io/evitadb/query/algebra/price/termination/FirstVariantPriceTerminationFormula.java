...
import io.evitadb.query.algebra.AbstractCacheableFormula;
...
import io.evitadb.query.algebra.price.filteredPriceRecords.FilteredPriceRecords.PriceRecordLookup;
...
import io.evitadb.query.extraResult.translator.histogram.producer.PriceHistogramProducer;
 ...
 *
 * This formula consumes and produces {@link Formula} of {@link PriceRecord#getEntityPrimaryKey() entity ids}. It uses
...
public class FirstVariantPriceTerminationFormula extends AbstractCacheableFormula implements FilteredPriceRecordAccessor, PriceTerminationFormula {
	 ...
	 * keys that were excluded due to {@link #priceFilter} constraint. This information is reused in
	...
	public FirstVariantPriceTerminationFormula(@Nonnull PriceHandlingContainerFormula containerFormula, @Nonnull PriceEvaluationContext priceEvaluationContext, @Nonnull QueryPriceMode queryPriceMode, @Nonnull PricePredicate priceFilter) {
		...
		this.priceRecordComparator = queryPriceMode == QueryPriceMode.WITH_VAT ?
			Comparator.comparingInt(PriceRecordContract::getPriceWithVat) :
			Comparator.comparingInt(PriceRecordContract::getPriceWithoutVat);
	}
	...
	public Formula getCloneWithInnerFormulas(@Nonnull Formula... innerFormulas) {
		Assert.isTrue(innerFormulas.length == 1, () -> new IllegalStateException("Expected exactly single delegate inner formula!"));
			...
			while (entityIdIterator.hasNext()) {
				...
				for (int i = 0; i < entityIdIterator.getPeek(); i++) {
					...
					for (final PriceRecordLookup priceRecords : priceRecordIterators) {
						priceRecords.forEachPriceOfEntity(
							...
							foundPrice -> {
								...
								final int innerRecordId = foundPrice.getInnerRecordId();
					...
					Assert.isTrue(
						...
						() -> new IllegalStateException("Price for entity with PK " + entityId + " unexpectedly not found!")
					);
