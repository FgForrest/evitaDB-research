...
class PriceListCompositionTerminationVisitor implements FormulaVisitor {
	...
	public void visit(Formula formula) {
		...
		if (processedFormula instanceof PriceHandlingContainerFormula) {
			final PriceHandlingContainerFormula containerFormula = (PriceHandlingContainerFormula) processedFormula;
			...
			switch (innerRecordHandling) {
				case NONE:
					convertedFormula = priceFilter == null ?
				...
				case FIRST_OCCURRENCE:
					convertedFormula = new FirstVariantPriceTerminationFormula(
						containerFormula, priceEvaluationContext, queryPriceMode,
						ofNullable(priceFilter).orElse(PricePredicate.NO_FILTER)
					);
					break;
				case SUM:
					convertedFormula = new SumPriceTerminationFormula(
						containerFormula, priceEvaluationContext, queryPriceMode,
						ofNullable(priceFilter).orElse(PricePredicate.NO_FILTER)
					);
					break;
				default:
					throw new IllegalStateException("Unsupported inner record handling type: " + innerRecordHandling);
			}
		} else {
