...
/**
 * This implementation of {@link FilteringConstraintTranslator} converts {@link IsTrue} to {@link AbstractFormula}.
 *
 * @author Jan Novotný (novotny@fg.cz), FG Forrest a.s. (c) 2021
 */
public class IsTrueTranslator implements FilteringConstraintTranslator<IsTrue> {

	@Override
	@Nonnull
	public Formula translate(@Nonnull IsTrue filterConstraint, @Nonnull FilterByVisitor filterByVisitor) {
		...
		if (attributeDefinition.isUnique()) {
			return new AttributeFormula(
				...
				filterByVisitor.applyOnUniqueIndexes(
					attributeDefinition, index -> new ConstantFormula(new ArrayBitmap(index.getRecordIdByUniqueValue(Boolean.TRUE)))
				)
			);
		} else {
			return new AttributeFormula(
				...
				filterByVisitor.applyOnFilterIndexes(
					attributeDefinition, index -> index.getRecordsEqualToFormula(Boolean.TRUE)
				)
			);
		}
	}
