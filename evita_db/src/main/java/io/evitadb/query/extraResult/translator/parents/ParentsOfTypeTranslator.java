...
/**
 * This implementation of {@link RequireConstraintTranslator} converts {@link ParentsOfType} to {@link ParentsProducer}.
 * The producer instance has all pointer necessary to compute result. All operations in this translator are relatively
 * cheap comparing to final result computation, that is deferred to {@link ExtraResultProducer#fabricate(io.evitadb.api.EvitaSession, DataChunk)} method.
 *
 * This translator interoperates with {@link ParentsTranslator} and shares/reuses same {@link ParentsProducer} instance.
 *
 * @author Jan Novotný (novotny@fg.cz), FG Forrest a.s. (c) 2022
 */
public class ParentsOfTypeTranslator implements RequireConstraintTranslator<ParentsOfType>, SelfTraversingTranslator {

	@Override
	public ExtraResultProducer apply(ParentsOfType parentConstraint, ExtraResultPlanningVisitor extraResultPlanner) {
		...
		ParentsProducer parentsProducer = extraResultPlanner.findExistingProducer(ParentsProducer.class);
		// for each requested entity type
		final Serializable[] entityTypes = parentConstraint.getEntityTypes();
		for (Serializable entityType : entityTypes) {
			...
			if (parentsProducer == null) {
				// if no ParentsProducer exists yet - create new one
				parentsProducer = new ParentsProducer(
					extraResultPlanner.getQueryContext(),
					reqEntityType -> evitaRequest.isRequiresEntityReferences() && entityReferenceSet.isEmpty() || entityReferenceSet.contains(entityType),
					entityType,
					true,
					referenceFetcher,
				...
				parentsProducer.addRequestedParentsIncludingSelf(
					entityType,
					extraResultPlanner.getGlobalEntityIndex(entityType),
					parentConstraint.getRequirements()
				);
