 ...
 *
 * The need is determined by the number of opened read write {@link io.evitadb.api.EvitaSession} to the catalog. If there
...
public class ObservableOutputKeeper {
	...
	public long getLockTimeoutSeconds() {
		return options.getLockTimeoutSeconds();
	}
	 ...
	 */
	@Nonnull
	public ObservableOutput<FileOutputStream> getObservableOutputOrCreate(Path targetFile, BiFunction<Path, StorageOptions, ObservableOutput<FileOutputStream>> createFct) {
	 ...
	 */
	public void free(Path targetFile) {
		try {
			mutex.lock();
