...
import static io.evitadb.storage.kryo.ObservableOutput.TAIL_MANDATORY_SPACE;
...
public class ObservableInput<T extends InputStream> extends Input {
	 ...
	 * of the record defined by {@link #expectedLength}. This constraint is not known to Kryo - the limit represents
	...
	public void seek(FileLocation location) {
		...
		((RandomAccessFileInputStream) this.inputStream).seek(location.getStartingPosition());
		this.limit = 0;
		this.actualLimit = -1;
		this.position = 0;
		this.total = 0;
		...
		this.readingPayload = false;
		this.overflowing = false;
		this.capacity = Math.min(this.buffer.length, location.getRecordLength());
	}
	 ...
	 * record.
	 *
	 * We need to constraint the `limit` to copy the end of the record because the limit is used in many Kryo methods
