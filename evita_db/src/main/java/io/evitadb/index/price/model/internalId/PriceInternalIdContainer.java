...
/**
 * Interface allow accessing internally assigned price identificators. We use our own identificators because we need
 ...
 * to keep in memory, and we would be forced to use {@link Roaring64Bitmap} which are considerably slower and even slower
 * for data generated by {@link NumberUtils#join(int, int)}, because there are big gaps between two adjoining entity
 * identificators. By mapping external ids to internally assigned int ids we could stick to the int type and allow
 * the required relation between identificators and entity.
 *
