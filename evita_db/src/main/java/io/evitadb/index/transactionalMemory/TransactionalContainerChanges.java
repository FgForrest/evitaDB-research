...
public final class TransactionalContainerChanges<DIFF_PIECE, COPY, PRODUCER extends TransactionalLayerProducer<DIFF_PIECE, COPY>> {
	...
	public void clean(TransactionalLayerMaintainer transactionalLayer) {
		getCreatedAndRemovedItems().forEach(it -> {
			final DIFF_PIECE memoryLayer = transactionalLayer.removeTransactionalMemoryLayerIfExists(it);
			it.createCopyWithMergedTransactionalMemory(memoryLayer, transactionalLayer, null);
		});
	}
	...
	public void cleanAll(TransactionalLayerMaintainer transactionalLayer) {
		final Consumer<PRODUCER> cleaningFct = item -> {
			final DIFF_PIECE memoryLayer = transactionalLayer.removeTransactionalMemoryLayerIfExists(item);
			item.createCopyWithMergedTransactionalMemory(memoryLayer, transactionalLayer, null);
		};
