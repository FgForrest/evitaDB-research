...
import io.evitadb.query.algebra.deferred.BitmapSupplier;
...
/**
 * Implementation of {@link BitmapSupplier} that provides access to the data stored in {@link HistogramIndex}
...
public class HistogramBitmapSupplier<T extends Comparable<T>> implements BitmapSupplier {
	...
	private final HistogramBucket<T>[] histogramBuckets;
	...
	public Bitmap get() {
		...
		Arrays.stream(histogramBuckets)
			.map(HistogramBucket::getRecordIds)
			.map(Bitmap::getArray)
