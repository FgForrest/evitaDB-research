...
public class IntArrayChanges implements ArrayChangesIteratorSupport {
	...
	void addRecordId(int recordId) {
		...
		if (position.isAlreadyPresent()) {
			int removalIndex = Arrays.binarySearch(removals, position.getPosition());
			if (removalIndex >= 0) {
			...
			final int index = Arrays.binarySearch(insertions, position.getPosition());
			if (index >= 0) {
				...
				this.insertions = ArrayUtils.insertIntIntoArrayOnIndex(position.getPosition(), this.insertions, startIndex);
	...
	void removeRecordId(int recordId) {
		...
		if (position >= 0) {
			...
			final int changePosition = ArrayUtils.computeInsertPositionOfIntInOrderedArray(recordId, this.delegate).getPosition();
