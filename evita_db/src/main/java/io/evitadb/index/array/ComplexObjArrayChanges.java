	...
	private int addRecordToExistingSetAtInsertionPoint(T recordId, int index) {
		...
		if (innerPosition.isAlreadyPresent()) {
			...
			if (combiner != null) {
				...
				recordId.removeFromTransactionalMemory();
			}
		} else {
			// there is no existing record, add it on the target place
			final T[] newInsertedValues = ArrayUtils.insertRecordIntoArray(recordId, insertedValuesBefore, innerPosition.getPosition());
	...
	private void reduceRemovalOrder(T recordId, int position) {
		...
		if (removalIndex >= 0) {
			...
			if (reducedValue == null || (obsoleteChecker != null && obsoleteChecker.test(reducedValue))) {
				...
				ofNullable(reducedValue).ifPresent(TransactionalObject::removeFromTransactionalMemory);
