...
import static io.evitadb.api.utils.ArrayUtils.computeInsertPositionOfIntInOrderedArray;
...
public class UnorderedLookup implements Serializable {
	...
	public void addRecord(int previousRecordId, int recordId) {
		...
		if (insertionPosition.isAlreadyPresent()) {
			...
			newPosition = this.positions[insertionPosition.getPosition()] + 1;
		...
		this.recordIds = ArrayUtils.insertIntIntoArrayOnIndex(recordId, this.recordIds, leadingPosition.getPosition());
		// now place the new position into unordered array of positions on the same index as record on previous line
		this.positions = ArrayUtils.insertIntIntoArrayOnIndex(newPosition, this.positions, leadingPosition.getPosition());
	...
	public void addRecordOnIndex(int index, int recordId) {
		...
		this.recordIds = ArrayUtils.insertIntIntoArrayOnIndex(recordId, this.recordIds, leadingPosition.getPosition());
		// now place the new position into unordered array of positions on the same index as record on previous line
		this.positions = ArrayUtils.insertIntIntoArrayOnIndex(index, this.positions, leadingPosition.getPosition());
		// we have to reset memoized result - modification has occurred
		this.memoizedUnorderedArray = null;
	}
	...
	public void appendRecords(int[] newRecordIds) {
		...
		for (int newRecordId : sortedNewRecordIds) {
			...
			final int position = ins.getPosition();
