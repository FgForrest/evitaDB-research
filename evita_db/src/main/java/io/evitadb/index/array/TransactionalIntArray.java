...
public class TransactionalIntArray implements TransactionalLayerProducer<IntArrayChanges, int[]>, Serializable {
	...
	public int addReturningIndex(int recordId) {
		...
		if (layer == null) {
			...
			if (!insertionPosition.isAlreadyPresent()) {
				this.delegate = ArrayUtils.insertIntIntoArrayOnIndex(recordId, this.delegate, insertionPosition.getPosition());
			}
			return insertionPosition.getPosition();
		} else {
			layer.addRecordId(recordId);
	...
	@Override
	public int[] createCopyWithMergedTransactionalMemory(@Nullable IntArrayChanges layer, @Nonnull TransactionalLayerMaintainer transactionalLayer, Transaction transaction) {
		if (layer == null) {
			return this.delegate;
		} else {
			return layer.getMergedArray();
