 ...
 */
@Data
public class FacetGroupIndex implements TransactionalLayerProducer<FacetGroupIndexChanges, Map<Integer, Bitmap>>, EntityIndexDataStructure {
	 ...
	 */
	private final TransactionalMemoryMap<Integer, FacetIdIndex> facetIdIndexes;

	public FacetGroupIndex() {
		this.groupId = null;
		this.facetIdIndexes = new TransactionalMemoryMap<>(new HashMap<>());
	}
	...
	public FacetGroupIndex(@Nullable Integer groupId, @Nonnull Collection<FacetIdIndex> facetIdIndexes) {
		this.groupId = groupId;
		this.facetIdIndexes = new TransactionalMemoryMap<>(
			facetIdIndexes
				.stream()
				.collect(
					Collectors.toMap(
	...
	FacetGroupIndex(@Nullable Integer groupId, @Nonnull Map<Integer, FacetIdIndex> facetIdIndexes) {
		this.groupId = groupId;
		this.facetIdIndexes = new TransactionalMemoryMap<>(facetIdIndexes);
	}
	...
	public void clearTransactionalMemory() {
		for (FacetIdIndex facetIdIndex : facetIdIndexes.values()) {
			facetIdIndex.clearTransactionalMemory();
		}

		final FacetGroupIndexChanges changes = TransactionalMemory.getTransactionalMemoryLayerIfExists(this);
		...
		TransactionalMemory.removeTransactionalMemoryLayerIfExists(this.facetIdIndexes);
	}
	...
	public static class FacetGroupIndexChanges {
		...
		public void addCreatedItem(FacetIdIndex baseIndex) {
			items.addCreatedItem(baseIndex);
		}

		public void addRemovedItem(FacetIdIndex baseIndex) {
