...
/**
 * This tests verifies basic properties of {@link Not} constraint.
 *
...
class NotTest {
	...
	void shouldCreateViaFactoryClassWorkAsExpected() {
		...
		assertNotNull(not);
		assertEquals(1, not.getConstraintCount());
		assertEquals("abc", ((Equals)not.getConstraints()[0]).getAttributeName());
		assertEquals("def", ((Equals)not.getConstraints()[0]).getAttributeValue());
	}

	@Test
	void shouldRecognizeApplicability() {
		assertTrue(new Not(eq("abc", "def")).isApplicable());
		final Not hackedCopyByInternalAPI = (Not) new Not(eq("abc", "def")).getCopyWithNewChildren(new FilterConstraint[0]);
		assertFalse(hackedCopyByInternalAPI.isApplicable());
	}

	@Test
	void shouldRecognizeNecessity() {
		assertTrue(new Not(eq("abc", "def")).isNecessary());
		final Not hackedCopyByInternalAPI = (Not) new Not(eq("abc", "def")).getCopyWithNewChildren(new FilterConstraint[0]);
	...
	void shouldToStringReturnExpectedFormat() {
		...
		assertNotNull(not);
		assertEquals("not(equals('abc','\\\''))", not.toString());
	}
	...
	private static Not createNotConstraint(String value) {
		return not(new Equals("abc", value));
	}
