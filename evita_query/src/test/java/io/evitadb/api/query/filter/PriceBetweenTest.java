...
/**
 * This tests verifies basic properties of {@link PriceBetween} constraint.
 *
...
class PriceBetweenTest {
	...
	void shouldRecognizeApplicability() {
		assertFalse(priceBetween(null, null).isApplicable());
		assertFalse(priceBetween(null, new BigDecimal("1")).isApplicable());
		assertFalse(priceBetween(new BigDecimal("1"), null).isApplicable());
		assertTrue(priceBetween(new BigDecimal("1"), new BigDecimal("2")).isApplicable());
	}

	@Test
	void shouldToStringReturnExpectedFormat() {
		final PriceBetween priceBetween = priceBetween(new BigDecimal("1"), new BigDecimal("2"));
		assertEquals("priceBetween(1,2)", priceBetween.toString());
	}
	...
	void shouldConformToEqualsAndHashContract() {
		...
		assertNotEquals(priceBetween(new BigDecimal("1"), new BigDecimal("2")).hashCode(), priceBetween(null, null).hashCode());
