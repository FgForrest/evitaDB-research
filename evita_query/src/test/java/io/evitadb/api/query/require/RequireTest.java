...
class RequireTest {
	...
	void shouldCreateViaFactoryClassWorkAsExpected() {
		...
		assertEquals(2, require.getConstraintCount());
		assertArrayEquals(new String[] {"abc"}, ((AttributeHistogram)require.getConstraints()[1]).getAttributeNames());
	}
	...
	void shouldRecognizeNecessity() {
		assertTrue(new Require(facetSummary(), attributes()).isNecessary());
	...
	void shouldToStringReturnExpectedFormat() {
		...
		assertNotNull(require);
		assertEquals("require(entityBody(),attributeHistogram(20,'abc'))", require.toString());
	}
	...
	private static Require createRequireConstraint(String... values) {
		return require(
				Arrays.stream(values)
						.map(QueryConstraints::associatedData)
