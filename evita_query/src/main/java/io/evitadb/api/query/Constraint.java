...
public interface Constraint<T extends Constraint<T>> extends Serializable {
	...
	/**
	 * Returns list of all constraint arguments.
	 */
	 ...
	 * Every constraint should call method visit on a visitor. Containers are NOT responsible for iterating
	 * over children and calling accept on them. Iterating is responsibility of the visitor implementation.
	 */
	void accept(ConstraintVisitor<T> visitor);

	/**
	 * Reproduces representative String of this constraint.
	 * Should imitate standard string parser conversion.
	 */
	String toString();

	/**
	 * Creates clone of this constraint using new set of arguments.
	 */
	T cloneWithArguments(Serializable[] newArguments);
