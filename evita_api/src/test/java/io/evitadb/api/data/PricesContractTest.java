...
class PricesContractTest {
	...
	private static final ZonedDateTime MOMENT_2020 = ZonedDateTime.of(2020, 12, 31, 23, 59, 59, 0, ZoneId.systemDefault());
	private static final ZonedDateTime MOMENT_2011 = ZonedDateTime.of(2011, 12, 31, 23, 59, 59, 0, ZoneId.systemDefault());
	...
	@Test
	void shouldReturnSellingPriceForNoneStrategy() {
		...
		assertSellingPrice(3, null, prices, CZK, MOMENT_2020, REFERENCE, VIP, LOGGED_ONLY, BASIC);
		// reference price is not indexed, vip price has not fulfilled validity, basic is the first
		assertSellingPrice(1, null, prices, CZK, MOMENT_2020, REFERENCE, VIP, BASIC, LOGGED_ONLY);
		// reference price is not indexed, vip price has not fulfilled validity - no other available
		assertNoSellingPrice(prices, CZK, MOMENT_2020, REFERENCE, VIP);
		// reference price is not indexed, vip price has fulfilled validity and is first
		assertSellingPrice(5, null, prices, CZK, MOMENT_2011, REFERENCE, VIP, LOGGED_ONLY, BASIC);
		// reference price is not indexed, vip price has fulfilled validity but is last
		assertSellingPrice(3, null, prices, CZK, MOMENT_2011, REFERENCE, LOGGED_ONLY, BASIC, VIP);
		// reference price is not indexed, vip price has not fulfilled validity, logged only is the first - we ask for EUR
		assertSellingPrice(4, null, prices, EUR, MOMENT_2020, REFERENCE, VIP, LOGGED_ONLY, BASIC);
		// reference price is not indexed, vip price has not fulfilled validity, logged only is the first - we ask for GBP
		assertNoSellingPrice(prices, GBP, MOMENT_2011, REFERENCE, VIP, LOGGED_ONLY, BASIC);
	}

	@Test
	void shouldReturnSellingPriceForFirstOccurrence() {
		final PricesContract prices = new Prices(
			1,
			Arrays.asList(
		...
		// reference price is not indexed, vip price has not fulfilled validity, logged only is the first
		assertSellingPrice(3, 3, prices, CZK, MOMENT_2020, REFERENCE, VIP, LOGGED_ONLY, BASIC);
		// reference price is not indexed, vip price has not fulfilled validity, basic is the first
		assertSellingPrice(1, 3, prices, CZK, MOMENT_2020, REFERENCE, VIP, BASIC, LOGGED_ONLY);
		// reference price is not indexed, vip price has not fulfilled validity - no other available
		assertNoSellingPrice(prices, CZK, MOMENT_2020, REFERENCE, VIP);
		// reference price is not indexed, vip price has fulfilled validity and is first
		assertSellingPrice(5, 3, prices, CZK, MOMENT_2011, REFERENCE, VIP, LOGGED_ONLY, BASIC);
		// reference price is not indexed, vip price has fulfilled validity but is last
		assertSellingPrice(3, 3, prices, CZK, MOMENT_2011, REFERENCE, LOGGED_ONLY, BASIC, VIP);
		// reference price is not indexed, vip price has not fulfilled validity, logged only is the first - we ask for EUR
		assertSellingPrice(4, 3, prices, EUR, MOMENT_2020, REFERENCE, VIP, LOGGED_ONLY, BASIC);
		// reference price is not indexed, vip price has not fulfilled validity, logged only is the first - we ask for GBP
		assertNoSellingPrice(prices, GBP, MOMENT_2011, REFERENCE, VIP, LOGGED_ONLY, BASIC);
	}

	@Test
	void shouldReturnSellingPriceForFirstOccurrenceWhenOneProductHasPriceInPriceListWithBiggerPriorityButLowerPrice() {
		final PricesContract prices = new Prices(
			1,
			Arrays.asList(
		...
		assertSellingPrice(5, 2, prices, CZK, null, VIP, LOGGED_ONLY, BASIC);
	}

	@Test
	void shouldReturnSellingPriceForSum() {
		final PricesContract prices = new Prices(
			1,
			Arrays.asList(
				ArrayUtils.mergeArrays(
		...
		// reference price is not indexed, vip price has not fulfilled validity, logged only is the first
		// 80 + 80 * 2 + 80 * 0.5 = 280
		assertSellingPrice(new BigDecimal("280.0"), prices, CZK, MOMENT_2020, REFERENCE, VIP, LOGGED_ONLY, BASIC);
		// reference price is not indexed, vip price has not fulfilled validity, basic is the first
		// 100 + 100 * 2 + 100 * 0.5 = 350
		assertSellingPrice(new BigDecimal("350.0"), prices, CZK, MOMENT_2020, REFERENCE, VIP, BASIC, LOGGED_ONLY);
		// reference price is not indexed, vip price has not fulfilled validity - no other available
		assertNoSellingPrice(prices, CZK, MOMENT_2020, REFERENCE, VIP);
		// reference price is not indexed, vip price has fulfilled validity and is first
		// 60 + 60 * 2 + 60 * 0.5 = 210
		assertSellingPrice(new BigDecimal("210.0"), prices, CZK, MOMENT_2011, REFERENCE, VIP, LOGGED_ONLY, BASIC);
		// reference price is not indexed, vip price has fulfilled validity but is last
		// 80 + 80 * 2 + 80 * 0.5 = 280
		assertSellingPrice(new BigDecimal("280.0"), prices, CZK, MOMENT_2011, REFERENCE, LOGGED_ONLY, BASIC, VIP);
		// reference price is not indexed, vip price has not fulfilled validity, logged only is the first - we ask for EUR
		// 8 + 8 * 2 + 8 * 0.5 = 28
		assertSellingPrice(new BigDecimal("28.0"), prices, EUR, MOMENT_2020, REFERENCE, VIP, LOGGED_ONLY, BASIC);
		// reference price is not indexed, vip price has not fulfilled validity, logged only is the first - we ask for GBP
		assertNoSellingPrice(prices, GBP, MOMENT_2011, REFERENCE, VIP, LOGGED_ONLY, BASIC);
	}
	...
	private static void assertHasPriceInRange(BigDecimal from, BigDecimal to, PricesContract prices, Currency currency, ZonedDateTime moment, Serializable... priceLists) {
		assertTrue(prices.hasPriceInInterval(from, to, QueryPriceMode.WITHOUT_VAT, currency, moment, priceLists));
	}

	private static void assertHasPriceInRangeWithMultiplier(BigDecimal from, BigDecimal to, BigDecimal[] multiplier, PricesContract prices, Currency currency, ZonedDateTime moment, Serializable... priceLists) {
		for (BigDecimal m : multiplier) {
			assertTrue(prices.hasPriceInInterval(from.multiply(m), to.multiply(m), QueryPriceMode.WITHOUT_VAT, currency, moment, priceLists));
		}
	}

	private static void assertHasNotPriceInRange(BigDecimal from, BigDecimal to, PricesContract prices, Currency currency, ZonedDateTime moment, Serializable... priceLists) {
		assertFalse(prices.hasPriceInInterval(from, to, QueryPriceMode.WITHOUT_VAT, currency, moment, priceLists));
	}

	private static void assertSellingPrice(int expectedPriceId, Integer innerRecordId, PricesContract prices, Currency currency, ZonedDateTime moment, Serializable... priceLists) {
		final PriceContract sellingPrice = prices.getSellingPrice(
			currency, moment, priceLists
		);

		assertNotNull(sellingPrice);
		assertEquals(combineIntoId(innerRecordId, expectedPriceId), sellingPrice.getPriceId());
	}

	private static void assertSellingPrice(BigDecimal expectedPriceWithoutVat, PricesContract prices, Currency currency, ZonedDateTime moment, Serializable... priceLists) {
		final PriceContract sellingPrice = prices.getSellingPrice(
			currency, moment, priceLists
		);

		assertNotNull(sellingPrice);
		assertEquals(expectedPriceWithoutVat, sellingPrice.getPriceWithoutVat());
	}

	private static void assertNoSellingPrice(PricesContract prices, Currency currency, ZonedDateTime moment, Serializable... priceLists) {
		final PriceContract sellingPrice = prices.getSellingPrice(
			currency, moment, priceLists
		);

		assertNull(sellingPrice);
	}
