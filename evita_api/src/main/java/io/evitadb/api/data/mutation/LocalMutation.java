...
/**
 * Entity {@link Mutation} allows to execute mutation operations on {@link Entity} object itself. Each change is increases
 * {@link HierarchicalPlacement#getVersion()} by one, data removal only sets
 * tombstone flag on a specifict data and doesn't really remove it. Possible removal will be taken care of during
...
public interface LocalMutation<T, S extends Comparable<S>> extends Mutation, Comparable<LocalMutation<T, S>> {
	...
	/**
	 * Returns type of the object this mutation alters.
	 */
	@Nonnull
	Class<T> affects();
	 ...
	 */
	@Nonnull
	T mutateLocal(@Nullable T value);
