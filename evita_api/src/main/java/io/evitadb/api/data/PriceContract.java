...
public interface PriceContract extends Versioned, Droppable, Serializable, Comparable<PriceContract>, ContentComparator<PriceContract> {
	 ...
	 */
	default boolean isValid(@Nonnull ZonedDateTime atTheMoment) {
	...
	default boolean differsFrom(@Nullable PriceContract otherPrice) {
		...
		if (!Objects.equals(getPriceWithoutVat(), otherPrice.getPriceWithoutVat())) return true;
		if (!Objects.equals(getPriceWithVat(), otherPrice.getPriceWithVat())) return true;
		if (!Objects.equals(getVat(), otherPrice.getVat())) return true;
