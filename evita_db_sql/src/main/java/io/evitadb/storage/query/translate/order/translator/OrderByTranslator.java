...
/**
 * Implementation of {@link OrderConstraintTranslator} which converts constraint {@link OrderBy} to generic {@link SqlSortExpression}
 *
...
public class OrderByTranslator implements OrderConstraintTranslator<OrderBy> {
    ...
    @Nonnull
    @Override
    public SqlSortExpression translate(@Nonnull OrderBy constraint, @Nonnull OrderTranslatingContext ctx) {
        final List<SqlSortExpression> innerExpressions = ctx.getCurrentLevelExpressions();

        // add fallback ordering - order by primary key
        innerExpressions.add(new SqlSortExpression("", "filteredEntity.primaryKey asc nulls last", List.of()));
