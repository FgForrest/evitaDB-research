...
/**
 * Implementation of {@link FilterConstraintTranslator} which converts constraint {@link GreaterThan} to generic {@link SqlPart}
 *
...
public class GreaterThanTranslator extends AttributeFilterConstraintTranslator<GreaterThan> {
    ...
    @Override
    public SqlPart translate(@Nonnull GreaterThan constraint, @Nonnull FilterTranslatingContext ctx) {
        final AttributeSchema attributeSchema = ctx.getAttributeSchema(constraint.getAttributeName());
        Assert.isTrue(!attributeSchema.getType().isArray(), "Constraint greaterThan does not support array values filtering.");
