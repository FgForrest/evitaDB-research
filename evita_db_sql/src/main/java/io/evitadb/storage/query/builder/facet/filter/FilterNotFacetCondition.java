...
import io.evitadb.storage.query.builder.facet.AbstractNotFacetCondition;
...
/**
 * Facet condition with NOT relation for finding entities by selected facets
 *
 * @author Lukáš Hornych 2021
 */
public class FilterNotFacetCondition extends AbstractNotFacetCondition {

    private final FilterTranslatingContext ctx;

    public FilterNotFacetCondition(FilterTranslatingContext ctx, GroupReference group, String serializedEntityType, int... facetIds) {
    ...
    public SqlPart get() {
        final StringBuilder cteSqlBuilder = new StringBuilder()
                .append("select distinct reference.entity_id " +
        ...
        return new SqlPart("entity.entity_id != all (select entity_id from " + cteAlias + ")");
    }
    ...
    public FacetCondition combine(GroupReference group, String serializedEntityType, int... facetIds) {
        ...
        return new FilterNotFacetCondition(
                ctx,
                getGroup(),
