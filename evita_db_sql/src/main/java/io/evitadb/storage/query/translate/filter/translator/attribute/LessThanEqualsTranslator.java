...
/**
 * Implementation of {@link FilterConstraintTranslator} which converts constraint {@link LessThanEquals} to generic {@link SqlPart}
 *
...
public class LessThanEqualsTranslator extends AttributeFilterConstraintTranslator<LessThanEquals> {
    ...
    @Override
    public SqlPart translate(@Nonnull LessThanEquals constraint, @Nonnull FilterTranslatingContext ctx) {
        final AttributeSchema attributeSchema = ctx.getAttributeSchema(constraint.getAttributeName());
        Assert.isTrue(!attributeSchema.getType().isArray(), "Constraint lessThanEquals does not support array values filtering.");
